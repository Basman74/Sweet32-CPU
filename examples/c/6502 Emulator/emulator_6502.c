...
uint16_t *UART_status, *UART_eventack;
uint8_t textattr = 7;
...
uint8_t getchar(void) {
  uint8_t d;
  UART_status = 0x70000000; // UART access addr
  UART_eventack = 0x70000002; // UART Rx event reset addr
/*  wait_rx:
...
uint8_t opcode, oldstatus, accumop;

//EhBASIC I/O stuff
uint8_t curkey = 0, tempkey;
...
/*uint8_t read6502(uint16_t address) {
	...
	if (address > 0xBFFF) return(ROM[address - 0xC000]);
	return(0);
}*/
...
//#define read6502(address) ( (!address & 0x8000) ? RAM[address] : ((address == 0xF004) ? getchar() : ((address > 0xBFFF) ? ROM[address - 0xC000] : 0) ) )
//#define read6502(address) ( (address == 0xF004) ? getchar() : RAM[address] )
#define read6502(address) RAM[address]

/*#define write6502(address, value) {\
	if (!(address & 0x8000)) {\
		RAM[address] = value;\
...
//flag calculations
void zerocalc(uint16_t n) {
    if ((n) & 0x00FF) clearzero();
        else setzero();
}

void signcalc(uint16_t n) {
    if ((n) & 0x0080) setsign();
        else clearsign();
}

void carrycalc(uint16_t n) {
    if ((n) & 0xFF00) setcarry();
        else clearcarry();
}

void overflowcalc(uint16_t n, uint16_t m, uint16_t o) { // n = result, m = accumulator, o = memory
	uint16_t temp, temp2;
	temp = n ^ m;
	temp2 = n ^ o;
	temp = temp & temp2;
	temp = temp & 0x80;
    if (temp == 0x80) setoverflow();
        else clearoverflow();
}

void saveaccum(uint8_t n) {
	a = n & 0xFF;
}*/
...
void push16(uint16_t pushval) {
	...
	temp = BASE_STACK + sp;
	temp2 = pushval >> 8;
    write6502(temp, temp2);
	temp = BASE_STACK + ((sp - 1) & 0xFF);
    write6502(temp, pushval);
    sp -= 2;
}
...
static uint16_t getvalue() {
	uint16_t temp;
	//temp = addrtable[opcode];
	//temp = temp - (uint16_t)acc;
    if (accumop == 1) return(a);
        else {
			temp = read6502(ea);
			return(temp);
		}
...
static void putvalue(uint16_t saveval) {
	...
	//temp = addrtable[opcode];
	//temp = temp - (uint16_t)acc;
    if (accumop == 1) a = (uint8_t)(saveval & 0x00FF);
...
int main(void) {
	uint32_t i;
	for (i=0; i<256; i++) {
		addrtable[i] = (void *)((uint32_t)addrtable[i]+ PROGRAM_BASE) ;
		optable[i] = (void *)((uint32_t)optable[i]+ PROGRAM_BASE) ;
	}
	for (i=0; i<16384; i++) {
		RAM[i + 0xC000] = ROM[i];
	}
...
uint8_t ROM[16384] = {
	...
	 
};
