...
unsigned char program[8192]; // Allocate 8KBytes of system RAM for TinyBASIC use
...
{
  wait_tx:
	if(!(*GPIO_access & 0x0100)) goto wait_tx;
	*GPIO_access = c;
}
...
unsigned int d;
  ...
  wait_rx:
	if(!(*GPIO_access & 0x0200)) goto wait_rx;

	d = *GPIO_access & 0x000000FF;
	return(d);
}
...
void main(void) 
{
	...
	GPIO_access = 0x70000000; // UART access
